...
def highlight_square(screen, game_state, valid_moves, square_selected):
    if square_selected != () and game_state.is_valid_piece(square_selected[0], square_selected[1]):
        logger.debug("Square selected for highlighting: %s", square_selected)
...
def main():
    ...
    while True:
        try:
            ...
            logger.debug("Number of players input: %s", number_of_players)
            if int(number_of_players) == 1:
                ...
                logger.error("Invalid input for number of players.")
                print("Enter 1 or 2.\n")
        except ValueError:
            logger.error("Invalid input for number of players.")
            print("Enter 1 or 2.")
    ...
    if human_player == 'b':
        ...
        logger.info("AI move made: %s", ai_move)

    elif human_player == 'w':
        ai_move = ai.minimax_white(game_state, 3, -100000, 100000, True, Player.PLAYER_1)
        game_state.move_piece(ai_move[0], ai_move[1], True)
        logger.info("AI move made: %s", ai_move)
            ...
            if e.type == py.QUIT:
                ...
                if not game_over:
                    ...
                    if square_selected == (row, col):
                        ...
                        logger.debug("Square selected: %s", square_selected)
                    ...
                    if len(player_clicks) == 2:
                        ...
                        if (player_clicks[1][0], player_clicks[1][1]) not in valid_moves:
                            ...
                            logger.info("Player move made: %s -> %s", player_clicks[0], player_clicks[1])
                            ...
                            if human_player == 'w':
                                ...
                                game_state.move_piece(ai_move[0], ai_move[1], True)
                                logger.info("AI (white) move evaluated: %s", ai_move)

                            elif human_player == 'b':
                                ai_move = ai.minimax_black(game_state, 3, -100000, 100000, True, Player.PLAYER_1)
                                game_state.move_piece(ai_move[0], ai_move[1], True)
                                logger.info("AI (black) move evaluated: %s", ai_move)
